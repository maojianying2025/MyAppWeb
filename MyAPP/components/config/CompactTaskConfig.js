...
export default function CompactTaskConfig() {
  ...
  const handleTaskDragEnd = (result) => {
    ...
    items.forEach((task, index) => {
      updateMutation.mutate({
        id: task.id,
        data: { ...task, display_order: index, icon_color: task.iconColor }
      });
    });
  };
    ...
    <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-xs text-gray-500">{task.modules.length} modules</p>
                                </div>
                              </div>
                              <div className="flex gap-1 flex-shrink-0">
                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => openModuleDialog(task.id)}>
                                  <Plus className="w-3 h-3" />
                                </Button>
                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => startEditTask(task)}>
                                  <Edit className="w-3 h-3" />
                                </Button>
                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleDeleteTask(task.id)}>
                                  <Trash2 className="w-3 h-3" />
                                </Button>
                              </div>
                            </div>

                            <DragDropContext onDragEnd={(result) => handleModuleDragEnd(result, task.id)}>
                                        ...
                                        <Draggable key={`${module.moduleId}-${index}`} draggableId={`${module.moduleId}-${index}`} index={index}>
                                          {(provided) => (
                                            ...
                                            >
                                              ...
                                              <ModuleIcon className="w-3 h-3 text-gray-600 flex-shrink-0" />
        ...
        <DialogContent className="max-w-md">
          ...
          <form onSubmit={handleTaskSubmit} className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" size="sm" onClick={() => setIsTaskDialogOpen(false)}>
                Cancel
              </Button>
              <Button type="submit" size="sm">{editingTask ? "Update" : "Create"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isModuleDialogOpen} onOpenChange={setIsModuleDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="text-base">Select Modules</DialogTitle>
          </DialogHeader>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t">
            <Button type="button" variant="outline" size="sm" onClick={() => setIsModuleDialogOpen(false)}>
              Cancel
            </Button>
            <Button size="sm" onClick={handleModuleSubmit}>
