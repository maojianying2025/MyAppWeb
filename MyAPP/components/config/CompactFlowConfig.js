...
const {
  $r
} = window; // React.createElement 快捷方式
...
function CompactFlowConfigComponent() {
  ...
  tasks.map(task => {
    ...
    }, `${flow.nodes.length} steps`)), $r(Button, {
      size: "sm",
      variant: "ghost",
    ...
    $r('div', {
      ...
      }, nodeInfo.name), node.currentHandler && $r('div', {
        className: 'text-gray-500 text-xs'
      }, `Handler: ${node.currentHandler}`)), $r('div', {
  ...
  }, $r(DialogHeader, null, $r(DialogTitle, {
    className: 'text-base'
  }, editingNodeIndex !== null ? "Edit Step" : "Add Step")), $r('div', {
    className: 'space-y-3'
  },
  // 步骤类型选择
  $r('div', null, $r(Label, {
  ...
  })), $r(SelectContent, null, FLOW_NODES.map(node => $r(SelectItem, {
    key: node.id,
    value: node.id
  }, $r('div', {
  ...
  })), $r(SelectContent, null, $r(SelectItem, {
    value: "initiator"
  }, "Initiator"), $r(SelectItem, {
    value: "current"
  }, "Current Handler"), roles.map(role => $r(SelectItem, {
    key: role.id,
    value: role.name
  }, role.name))))),
  // 操作按钮
  ...
  // 操作配置
  nodeFormData.actions?.map((action, idx) => $r('div', {
    key: idx,
  ...
  $r('div', null, $r(Label, {
    className: 'text-xs'
  }, 'CC Roles (Carbon Copy)'), $r(Button, {
    type: "button",
    variant: "outline",
    size: "sm",
    className: "w-full mt-1",
    onClick: () => setShowCCConfig(!showCCConfig)
  }, `${showCCConfig ? "Hide" : "Configure"} CC (${nodeFormData.ccRoles?.length || 0})`), showCCConfig && $r('div', {
    className: 'grid grid-cols-2 gap-1 mt-2 p-2 border rounded max-h-32 overflow-y-auto'
  }, roles.map(role => {
    ...
    return $r('button', {
      ...
      onClick: () => {
        ...
        setNodeFormData({
          ...nodeFormData,
          ccRoles: isSelected ? ccRoles.filter(r => r !== role.name) : [...ccRoles, role.name]
        });
      },
      className: `p-1.5 text-xs border rounded ${isSelected ? 'border-blue-500 bg-blue-50' : ''}`
    }, role.name);
  }))),
  ...
  $r('div', null, $r(Label, {
    ...
    className: 'grid grid-cols-3 gap-1 max-h-48 overflow-y-auto mt-1 p-1 border rounded'
    ...
    return $r('button', {
      ...
      onClick: () => {
        ...
        setNodeFormData({
          ...nodeFormData,
          modules: isSelected ? modules.filter(m => m !== module.id) : [...modules, module.id]
        });
      },
      className: `p-1.5 border rounded text-left hover:bg-gray-50 ${isSelected ? 'border-blue-500 bg-blue-50' : ''}`
    }, $r('div', {
